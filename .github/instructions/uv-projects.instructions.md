...
For this Agent Sandbox project, also include:
...
```bash
uv init
uv add ipykernel ipywidgets ruff tqdm pytest pytest-asyncio [additional packages]
uv sync
uv lock
```
...
Use dependency groups to separate development and production dependencies:
...
In pyproject.toml, this creates:
...
This ensures the virtual environment is activated and all dependencies are available.
